...
void play() {
	...
	switch (state) {
		...
		case AEL_STATE_PAUSED:
			...
			break;
		default: ESP_LOGI(TAG, "[ * ] Not supported state %d", state);
	}
}
...
vvoid sdcard_url_save_cb(void *user_data, char *url) {
	playlist_operator_handle_t sdcard_handle =
	    (playlist_operator_handle_t)user_data;
...
void initialize(esp_periph_set_handle_t periph_set) {
	...
	sdcard_scan(sdcard_url_save_cb, "/sdcard", 0, (const char *[]){ "mp3" }, 1,
	            sdcard_list_handle);
	...
	/* ZL38063 audio chip on board of ESP32-LyraTD-MSC does not support 44.1 kHz
	   sampling frequency, so resample filter has been added to convert audio
	   data to other rates accepted by the chip. You can resample the data to 16
	   kHz or 48 kHz.
	*/
	...
	ESP_LOGI(TAG, "[4.6] Link it together "
	              "[sdcard]-->fatfs_stream-->mp3_decoder-->resample-->i2s_"
	              "stream-->[codec_chip]");
